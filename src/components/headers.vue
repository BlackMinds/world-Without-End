<template>
    <div class="headers">
        <div class="head-cen">
            ...
            <!-- 真灵 -->
            <Modal width="720" title="真灵" v-model="PetsEject" :styles="{ top: '100px' }">
                <Tabs v-if="PetsList" :value="Pets" @click.native="cs">
                    <TabPane v-for="item in PetsList" :key="item.auraId" :label="
            ...
            <!-- 组队 -->
            <Modal width="720" :title="organizeList.armyName ? organizeList.armyName : '组队'" v-model="organizeEject" :styles="{ top: '100px' }" class="organizeEject">
                <div v-for="(item, idx) in organizeList.charaInfoVoList" :key="idx" class="organizeList">
                    <div v-if="item.avatar" style="position: relative; float: left">
                        <img :src="item.avatar" alt="老弟,没图片找群主要啊" class="organizeList_face" />
            ...
            ">
                ...
                <Button @click="CombatRecord()" :loading="recordStatus" style="margin-top: 10px">
                    战斗记录
                </Button>
                <template v-if="combatRecordProcess">
                    <p v-for="(item, idx) in combatRecordProcess" :key="item.reHealth" style="font-size: 18px; margin: 5px 0">
                        第{{ ++idx }}场战斗
                ...
                <div slot="footer">
                    <Button type="primary" size="small" @click="join" v-if="teamStatus == 0">
                        队伍列表
                    </Button>
                    <Button type="primary" size="small" @click="establish" v-if="teamStatus == 0">
                        创建队伍
                    </Button>
                    <Button type="primary" size="small" @click="quitTheTeam" v-if="teamStatus == 1">
                        退出队伍
                    </Button>
                </div>
            </Modal>
            ...
            <!-- 队伍名字弹出框 -->
            <Modal width="320" title="队伍名字" v-model="armyEject" @on-ok="changeEstablish()" :styles="{ top: '100px' }">
                <div class="pasEject">
            ...
            <!-- 队伍列表弹出框 -->
            <Modal width="520" title="队伍列表" v-model="armyListEject" :styles="{ top: '100px' }">
                <div v-for="(item, idx) in armyList" :key="idx" style="margin-bottom: 5px">
                    队伍名:
            ...
            <!-- 设置 -->
            <Modal width="750" title="设置" v-model="setEject" @on-ok="changeSet()" :styles="{ top: '100px' }">
                <Form ref="formInline" :model="gamePackageBo" :label-width="80" inline>
                    <FormItem label="自动出售">
                        <i-switch v-model="gamePackageBo.isOpen" :true-value="1" :false-value="0" />
                    </FormItem>
                    <FormItem label="最低等级">
            ...
            <!-- 攻略弹出框 -->
            <Modal width="620" title="更新日志" v-model="strategy" @on-ok="this.strategy = false" :styles="{ top: '40px' }">
                <updateLog></updateLog>
            </Modal>

            <!-- 修改密码弹出框 -->
            <Modal width="620" title="修改密码" v-model="pasEject" @on-ok="confirmPassword()" :styles="{ top: '100px' }">
                <div class="pasEject">
            ...
            <!-- 排行榜 -->
            <Modal width="620" title="排行榜" v-model="PlanEject" :styles="{ top: '100px' }">
...
export default {
    ...
    methods: {
        ...
        out() {
            this.$http
                .post("/gamepassport/logout?userId=" +
                    this.getCookie("userId"))
                .then((res) => {
                    this.$Message.success(res.data.msg);
                })
                .catch((err) => {
                    this.$Message.warning("退出登录失败,请联系管理员");
                });
